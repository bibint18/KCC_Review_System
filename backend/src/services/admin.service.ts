...
export default class AdminService {
  async getAllUsers(skip: number = 0, limit: number = 100, role?: UserRole): Promise<any[]> {
    return userRepo.findAll(skip, limit, role);
  }

  async updateUserStatus(userId: string, isActive: boolean, isAvailable?: boolean): Promise<any> {
  ...
  async getWorkflowPerformance(days: number = 30): Promise<any> {
    ...
    const completedQuestions = await questionRepo.findByCreatedAtAndStatus(startDate, QuestionStatus.GOLDEN_FAQ_CREATED);
    ...
    const specialistPerformance = await User.aggregate([
      { $match: { role: UserRole.AGRI_SPECIALIST } },
      {
        $lookup: {
          ...
          let: { userId: '$_id' },
          pipeline: [
            { $match: { $expr: { $eq: ['$assigned_specialist_id', '$$userId'] }, created_at: { $gte: startDate } } },
      ...
      },
      {
        $lookup: {
          ...
          let: { userId: '$_id' },
          pipeline: [
            { $match: { $expr: { $eq: ['$created_by', '$$userId'] }, created_at: { $gte: startDate } } },
            { $group: { _id: null, count: { $sum: 1 } } },
          ],
      ...
      },
      {
        $project: {
          ...
          golden_faqs_created: { $arrayElemAt: ['$golden_faqs_created.count', 0] },
        },
      },
    ...
    return {
      ...
      completion_rate: totalQuestions.length > 0 ? (completedQuestions.length / totalQuestions.length) * 100 : 0,
